...
<aside class="sidebar fixed top-0 bottom-0 left-0 z-[60] hidden lg:flex flex-col p-10">
    <div class="mb-6 text-main text-left">
        @if($profile->image)
        <img src="{{ strpos($profile->image, 'http') === 0 ? $profile->image : asset('storage/' . $profile->image) }}" alt="Profile" class="w-full aspect-square rounded-2xl object-cover mb-6 shadow-sm">
        @else
        <img src="{{ asset('images/profile.png') }}" alt="Profile" class="w-full aspect-square rounded-2xl object-cover mb-6 shadow-sm">
        @endif
